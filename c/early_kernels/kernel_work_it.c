...
void keyboard_handler_main(void)
{
	...
	if (status & 0x01) {
		...
		if(keycode == ENTER_KEY_CODE) {
			kprint_newline();
			const char *str1 = "You have entered the string: ";
			kprint(str1);
			...
			kprint_newline();
			kprint_newline();
			const char *str2 = "Enter a new string: ";
			kprint(str2);
		...
		if (length_entered_sentense >=max_length -1) 
			{	
				kprint_newline();
				const char *str1 = "Length of your message more then MAX_LENGTH (19 symbols)";
				kprint(str1);
				kprint_newline();
				const char *str2 = "Your message before clearing: ";
				kprint(str2);
				//печатаем введенную строку (то, что было введено до того, как был превышен максимальный размер сообщения)
				kprint((const char *)msg);
				kprint_newline();
				clear_buffer_string();
				const char *str3 = "Your message has been cleared :(";
				kprint(str3);
				kprint_newline();
				kprint_newline();
				const char *str4 = "Enter a new string: ";
				kprint(str4);
...
void kmain(void)
{
	...
	const char *str1 = "Enter a new string: ";
	kprint(str1);
